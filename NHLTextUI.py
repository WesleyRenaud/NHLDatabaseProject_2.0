...
class NHLTextUI:
    ...
    def add_player( self ):
        ...
        if player_type == 'skater' or player_type == 'Skater':
            ...
            draft_position = input( " * Enter the player's draft position or 'None' if they went undrafted >> " )
            player = NHL.Skater( name, team, number, position, height, weight, birthday, handedness, 
                                 draft_position )
            ...
            while not finished_adding_seasons: # loop until the user is done adding their seasons
                ...
                time_on_ice_per_game = (float)(input( " * Enter the time on ice/game during the season >> " ))
            ...
            while not finished_adding_seasons: # loop until the user is done adding their seasons
                ...
                loses = (int)(input( " * Enter the number of loses during the season >> " ))
                ties = (int)(input( " * Enter the number of ties during the season or 'None' if there are no ties in this season >> " ))
                overtime_loses = (int)(input( " * Enter the number of overtime loses during the season or 'None' if there are no " + 
                                        " overtime loses in the season >> " ))
                ...
                player.add_season( season, games_played, games_started, wins, loses, ties, overtime_loses, 
            ...
            while not finished_adding_playoffs: # loop until the user is done adding their seasons
                ...
                penalty_minutes = (int)(input( " * Enter the number of penalty minutes taken during the season >> " ))
                time_on_ice = (int)(input( " * Enter the time of ice during the season >> " ))
                player.add_playoffs( season, games_played, games_started, wins, loses, ties, overtime_loses, 
                                     shots_against, goals_against_average, save_percentage, shutouts, goals, 
                                     assists, penalty_minutes, time_on_ice )
    ...
    def add_standings( self ):
        ...
        for i in range( num_teams ):
            ...
            loses = (int)(input( " * Enter the number of loses for %s >> " % (city + " " + name) ))
            overtime_loses = (int)(input( " * Enter the number of overtime loses for %s or 'None' if there are no overtime " +
                                    " loses during the season >> " % (city + " " + name) ))
            ...
            teams.append( NHL.team( city, name, games_played, wins, loses, overtime_loses, ties, points, 
